...
// unordered_mapのキーとしてpair<int,int>を直接使うよりも高速かつ簡潔。
long long encode(int row, int col, int numCols) {
    return static_cast<long long>(row) * numCols + col;
}

// Octile distance（許容的ヒューリスティック）。
// 8方向移動（直進コスト1、斜めコストsqrt(2)）における
// 2点間の理論上の最短到達コストを返す。
...
std::vector<std::pair<int, int>> astar(const std::vector<std::vector<int>>& grid,
    ...
    std::unordered_map<long long, double> gScore; // スタートからの確定済み最小実コスト
    std::unordered_map<long long, long long> parent; // 経路復元用の親ノード
    std::unordered_map<long long, bool> closed; // 確定済み（最短距離が確定した）ノード
    ...
    while (!openQueue.empty()) {
        ...
        if (closed[currentKey]) {
            continue; // 古い（無効化された）キューエントリはスキップ
        }
        ...
        for (int d = 0; d < kDirCount; ++d) {
            ...
            if (it == gScore.end() || tentativeG < it->second) {
                ...
                const double f = tentativeG + octileDistance({newRow, newCol}, goal);
