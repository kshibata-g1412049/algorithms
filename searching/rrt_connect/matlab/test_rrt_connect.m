function test_rrt_connect()
% TEST_RRT_CONNECT rrt_connect関数のテストスクリプト（MATLAB/Octave両対応）。
%
...
%   全テストが成功すれば "All tests passed." が表示される。
    ...
    for seed = [1, 2, 3]
        path = rrt_connect(no_obstacles, bounds, start, goal, goal_radius, step_size, ...
            max_iterations, seed);
    ...
    with_obstacle = [5, 5, 2];
    for seed = [1, 2, 3]
        path = rrt_connect(with_obstacle, bounds, start, goal, goal_radius, step_size, ...
            max_iterations, seed);
    ...
    path = rrt_connect(no_obstacles, bounds, start, goal, goal_radius, step_size, 1, 42);
