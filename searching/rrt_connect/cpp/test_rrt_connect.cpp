// rrt_connect.cppのテスト。
//
// このファイルは同じフォルダ内のrrt_connect.h/.cppのみに依存しており、
...
//      （RRT-Connectは双方向探索のため通常のRRTより小さいmaxIterationsでも
...
bool checkPathProperties(
    ...
    // 4. 経路上の各線分が障害物と衝突していない（接続点をまたぐ線分も含む）
...
int main() {
    ...
    // 1, 2, 3, 4, 5: 障害物なしのシンプルなシナリオを複数シードで検証
    {
        ...
        for (unsigned int seed : {1u, 2u, 3u}) {
            const auto path = rrtConnect(obstacles, bounds, start, goal, goalRadius,
    ...
    // 7: maxIterationsが極端に小さい場合（空、または性質を満たす経路のみ許容）
    {
        std::vector<std::tuple<double, double, double>> obstacles;
        const auto path =
            rrtConnect(obstacles, bounds, start, goal, goalRadius, stepSize, 1, 42u);
