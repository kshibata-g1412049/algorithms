"""rrt_connect関数のテスト（pytest）。

このファイルは同じディレクトリ内のrrt_connect.pyのみに依存しているため、
python/フォルダを単独で取り出してもこのテストは実行できる。

RRT-Connectは乱数を使うアルゴリズムであり、固定シードでもC++/Python/MATLAB
間で同じ乱数列にはならない（言語ごとに乱数生成器の実装が異なるため）。
そのため、ここでは出力の厳密な値ではなく、以下の「性質」を検証する
プロパティベーステストを行う:
  1. 十分大きなmax_iterationsであれば、複数の固定シードで経路が見つかる
     （RRT-Connectは双方向探索のため通常のRRTより小さいmax_iterationsでも
      十分なはずだが、安定性を考慮した値を使う）。
  2. 経路の先頭がstartと一致する。
  3. 経路の末尾がgoalからgoal_radius以内にある。
  4. 経路上の各線分（2つの木を結合した接続部分も含む）が障害物と
     衝突していない。
...
def _assert_valid_path(path, start, goal, goal_radius, step_size, obstacles):
    ...
    # 4, 5. 各線分（接続部分も含む）が障害物と衝突していない & 各ステップが
    # step_size以下
...
def test_no_obstacles_finds_path(seed):
    ...
    obstacles = []
    path = rrt_connect(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                        MAX_ITERATIONS, seed)
...
def test_tiny_max_iterations_returns_empty_or_valid():
    ...
    obstacles = []
    path = rrt_connect(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE, 1, 42)
    if path:
        _assert_valid_path(path, START, GOAL, GOAL_RADIUS, STEP_SIZE, obstacles)
    else:
        assert path == []
...
def test_seed_reproducibility():
    ...
    obstacles = []
    path1 = rrt_connect(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                         MAX_ITERATIONS, 7)
    path2 = rrt_connect(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                         MAX_ITERATIONS, 7)
    assert path1 == path2
