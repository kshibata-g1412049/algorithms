"""fibonacci_search関数のテスト（pytest）。

このファイルは同じディレクトリ内のfibonacci_search.pyのみに依存しているため、
...
def test_single_element_no_match():
    """要素1つの配列：一致しない場合"""
    assert fibonacci_search([42], 7) == -1
...
def test_target_at_tail():
    """targetが末尾にある場合"""
    arr = [1, 3, 5, 7, 9, 11, 13, 15, 17]
    assert fibonacci_search(arr, 17) == 8


def test_target_at_middle():
    """targetが中央にある場合"""
    arr = [1, 3, 5, 7, 9, 11, 13, 15, 17]
    assert fibonacci_search(arr, 9) == 4
...
def test_target_not_found():
    ...
    assert fibonacci_search(arr, 100) == -1
    assert fibonacci_search(arr, 45) == -1
...
def test_two_elements():
    """2要素の配列（境界値の確認）"""
    arr = [3, 8]
    assert fibonacci_search(arr, 3) == 0
    assert fibonacci_search(arr, 8) == 1
