% test_bidirectional_astar.m
%
% bidirectional_astar関数のテストスクリプト（MATLAB/Octave両対応）。
% このフォルダを単独で取り出してそのまま実行できる（外部依存なし）。
%
% 実行方法: MATLAB/Octave上でこのスクリプトを実行する。
%   >> test_bidirectional_astar
...
startPos = [1 1];
goalPos = [5 5];
path1 = bidirectional_astar(grid1, startPos, goalPos);
...
startPos = [1 1];
goalPos = [5 5];
path2 = bidirectional_astar(grid2, startPos, goalPos);
...
% --- startとgoalが同一の場合 ---
grid3 = zeros(3, 3);
path3 = bidirectional_astar(grid3, [2 2], [2 2]);
...
path4 = bidirectional_astar(grid4, [1 1], [5 5]);
...
path5 = bidirectional_astar(grid5, [1 1], [3 3]);
