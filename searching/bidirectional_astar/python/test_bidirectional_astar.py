"""bidirectional_astar関数のテスト（pytest）。

このファイルは同じディレクトリ内のbidirectional_astar.pyのみに依存しているため、
...
def test_start_equals_goal():
    ...
    path = bidirectional_astar(grid, (1, 1), (1, 1))
    assert path == [(1, 1)]
...
def test_unreachable_goal():
    ...
    assert bidirectional_astar(grid, (0, 0), (4, 4)) == []
...
def test_corner_cut_blocked():
    ...
    assert bidirectional_astar(grid, (0, 0), (2, 2)) == []
