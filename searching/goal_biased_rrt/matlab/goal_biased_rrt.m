function path = goal_biased_rrt(obstacles, bounds, start, goal, goal_radius, step_size, max_iterations, seed)
% GOAL_BIASED_RRT ゴールバイアス付き高速探索ランダム木（Goal-biased RRT）による経路探索
%
...
%
%   path = GOAL_BIASED_RRT(obstacles, bounds, start, goal, goal_radius, step_size, max_iterations, seed)
%
...
%         見つからない場合は空行列[]。
    ...
    % goalをサンプル点として選ぶ確率（ゴールバイアス）。残り
    % (1 - goal_bias_probability) の確率では探索空間内に完全ランダムな点を
    % サンプリングする。
