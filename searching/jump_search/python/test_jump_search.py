"""jump_search関数のテスト（pytest）。

このファイルは同じディレクトリ内のjump_search.pyのみに依存しているため、
...
def test_target_at_tail():
    """targetが末尾にある場合"""
    arr = [1, 3, 5, 7, 9, 11, 13, 15, 17]
    assert jump_search(arr, 17) == 8


def test_target_at_middle():
    """targetが中央にある場合"""
    arr = [1, 3, 5, 7, 9, 11, 13, 15, 17]
    assert jump_search(arr, 9) == 4
