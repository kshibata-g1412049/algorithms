"""exponential_search関数のテスト（pytest）。

このファイルは同じディレクトリ内のexponential_search.pyのみに依存しているため、
...
def test_target_at_tail():
    """targetが末尾にある場合"""
    arr = [1, 3, 5, 7, 9, 11, 13, 15, 17]
    assert exponential_search(arr, 17) == 8
...
def test_target_not_found():
    ...
    assert exponential_search(arr, 45) == -1
