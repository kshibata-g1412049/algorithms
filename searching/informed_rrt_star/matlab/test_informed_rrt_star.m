function test_informed_rrt_star()
% TEST_INFORMED_RRT_STAR informed_rrt_star関数のテストスクリプト（MATLAB/Octave両対応）。
%
%   このフォルダを単独で取り出してそのまま実行できる（外部依存なし）。
%
%   Informed RRT*は乱数を使うアルゴリズムであり、固定シードでもC++/Python/MATLAB
%   間で同じ乱数列にはならない（言語ごとに乱数生成器の実装が異なるため）。
%   また本実装はmax_iterations回まで探索を継続し、見つかった中で最良コストの
...
%   全テストが成功すれば "All tests passed." が表示される。
    ...
    goal_radius = 0.5;
    step_size = 0.5;
    % Informed RRT*はRRT/RRT*と異なり最初の解で打ち切らずmax_iterations回
    % 必ず最後まで実行する（O(n^2)）ため、テストの実行時間を抑えるために
    % RRT/RRT*テストより小さいmax_iterationsを使う（経験上2000で両シナリオとも
    % 複数シードで安定して経路が見つかる）。
    max_iterations = 2000;
    ...
    for seed = [1, 2, 3]
        path = informed_rrt_star(no_obstacles, bounds, start, goal, goal_radius, ...
    ...
    with_obstacle = [5, 5, 2];
    for seed = [1, 2, 3]
        path = informed_rrt_star(with_obstacle, bounds, start, goal, goal_radius, ...
            step_size, max_iterations, seed);
    ...
    path = informed_rrt_star(no_obstacles, bounds, start, goal, goal_radius, step_size, 1, 42);
