"""Informed RRT*（Informed Rapidly-exploring Random Tree Star）の実装。
...
改良版のサンプリングベース経路計画アルゴリズム。各反復で以下を行う:
  ...
  5. 親選択（choose parent、RRT*と同じ）: 新しい点から一定の近傍半径以内に
     ...
     親候補・リワイヤ対象として実際に採用するのはstep_size以内のノードに限定
     している（近傍探索自体はstep_sizeより広い半径で行うが、採用条件を
...
def _sample_from_ellipse(c_min: float, c_best: float, center_x: float,
                          center_y: float, angle: float,
    ...
    回転・平行移動して楕円上の点に変換する。
    """
    # 長軸・短軸の半径。c_bestがc_minより十分大きい正常な場合のみ意味を持つ
    # （呼び出し側でc_best > c_minを保証する）。
    a = c_best / 2.0
    b_squared = (c_best * c_best - c_min * c_min) / 4.0
...
def informed_rrt_star(obstacles: List[Tuple[float, float, float]],
                       bounds: Tuple[float, float, float, float],
                       start: Tuple[float, float], goal: Tuple[float, float],
                       goal_radius: float, step_size: float,
                       max_iterations: int, seed: int) -> List[Tuple[float, float]]:
    """Informed RRT*で2次元連続空間上の経路を探索する。
    ...
    Returns:
        startからgoal付近までの、見つかった中で最良コストの経路
    ...
    c_min = _distance(start[0], start[1], goal[0], goal[1])
    ...
    for _ in range(max_iterations):
        ...
        if not math.isfinite(best_cost):
            ...
            sample_x = 0.0
            sample_y = 0.0
            for _attempt in range(max_ellipse_resample_attempts):
                point_x, point_y = _sample_from_ellipse(
                    c_min, best_cost, center_x, center_y, angle, rng)
        ...
        best_parent_cost = nearest_node.cost + _distance(
        ...
        for candidate_index in neighbors:
            ...
            if candidate_cost < best_parent_cost and _is_segment_collision_free(
                    candidate.x, candidate.y, new_x, new_y, obstacles):
                best_parent_cost = candidate_cost
                best_parent_index = candidate_index

        # 6. 新しい点をそのコストとともに木に追加する。
        nodes.append(_Node(new_x, new_y, best_parent_index, best_parent_cost))
        ...
        if _distance(new_x, new_y, goal[0], goal[1]) <= goal_radius:
            if nodes[new_index].cost < best_cost:
                best_cost = nodes[new_index].cost
    ...
    if best_goal_index is None:
        return []
    return _reconstruct_path(nodes, best_goal_index)
