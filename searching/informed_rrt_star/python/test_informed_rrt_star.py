"""informed_rrt_star関数のテスト（pytest）。

このファイルは同じディレクトリ内のinformed_rrt_star.pyのみに依存しているため、
...
GOAL_RADIUS = 0.5
STEP_SIZE = 0.5
# Informed RRT*はRRT/RRT*と異なり最初の解で打ち切らずmax_iterations回必ず
# 最後まで実行する（O(n^2)）ため、テストの実行時間を抑えるためにRRT/RRT*の
# テストより小さいmax_iterationsを使う（経験上2000で両シナリオとも複数シード
# で安定して経路が見つかる）。
MAX_ITERATIONS = 2000
...
def test_seed_reproducibility():
    ...
    obstacles = []
    path1 = informed_rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS,
                               STEP_SIZE, MAX_ITERATIONS, 7)
    path2 = informed_rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS,
                               STEP_SIZE, MAX_ITERATIONS, 7)
    assert path1 == path2
