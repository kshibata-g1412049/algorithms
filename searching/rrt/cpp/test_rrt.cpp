// rrt.cppのテスト。
//
// このファイルは同じフォルダ内のrrt.h/.cppのみに依存しており、
...
int main() {
    ...
    // 1, 2, 3, 4, 5: 障害物なしのシンプルなシナリオを複数シードで検証
    {
        ...
        for (unsigned int seed : {1u, 2u, 3u}) {
            const auto path = rrt(obstacles, bounds, start, goal, goalRadius, stepSize,
                                   maxIterations, seed);
    ...
    {
        std::vector<std::tuple<double, double, double>> obstacles;
        const auto path = rrt(obstacles, bounds, start, goal, goalRadius, stepSize, 1, 42u);
