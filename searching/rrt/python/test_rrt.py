...
このファイルは同じディレクトリ内のrrt.pyのみに依存しているため、
...
def test_no_obstacles_finds_path(seed):
    ...
    obstacles = []
    path = rrt(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
               MAX_ITERATIONS, seed)
...
def test_tiny_max_iterations_returns_empty_or_valid():
    ...
    obstacles = []
    path = rrt(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE, 1, 42)
    if path:
        _assert_valid_path(path, START, GOAL, GOAL_RADIUS, STEP_SIZE, obstacles)
    else:
        assert path == []
...
def test_seed_reproducibility():
    ...
    obstacles = []
    path1 = rrt(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                MAX_ITERATIONS, 7)
    path2 = rrt(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                MAX_ITERATIONS, 7)
    assert path1 == path2
