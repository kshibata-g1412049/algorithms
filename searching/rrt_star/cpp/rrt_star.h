...
//      実際に採用するのはstepSize以内のノードに限定している（近傍探索自体は
//      stepSizeより広い半径で行うが、採用条件をstepSize以内に絞ることで、
//      木のどの辺もstepSize以下に保たれ、基本RRTと同じ「1ステップの移動距離は
//      stepSize以下」という性質を保てる。本来のRRT*はこの制約を置かないが、
//      経路検証を単純にするための学習用途の簡略化）。
//   6. 新しい点をそのコストとともに木に追加する。
//   7. リワイヤ（rewire、RRT*独自）: 新しいノードの近傍集合（手順5と同じ近傍半径、
//      ただし採用はstepSize以内に限定）にある既存の各ノードについて、
