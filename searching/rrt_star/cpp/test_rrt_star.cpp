// rrt_star.cppのテスト。
//
// このファイルは同じフォルダ内のrrt_star.h/.cppのみに依存しており、
...
int main() {
    ...
    // 1, 2, 3, 4, 5: 障害物なしのシンプルなシナリオを複数シードで検証
    {
        ...
        for (unsigned int seed : {1u, 2u, 3u}) {
            const auto path = rrtStar(obstacles, bounds, start, goal, goalRadius, stepSize,
                                       maxIterations, seed);
    ...
    // 7: maxIterationsが極端に小さい場合（空、または性質を満たす経路のみ許容）
    {
        std::vector<std::tuple<double, double, double>> obstacles;
        const auto path =
            rrtStar(obstacles, bounds, start, goal, goalRadius, stepSize, 1, 42u);
