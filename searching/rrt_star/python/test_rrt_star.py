"""rrt_star関数のテスト（pytest）。

このファイルは同じディレクトリ内のrrt_star.pyのみに依存しているため、
python/フォルダを単独で取り出してもこのテストは実行できる。

RRT*は乱数を使うアルゴリズムであり、固定シードでもC++/Python/MATLAB間で
同じ乱数列にはならない（言語ごとに乱数生成器の実装が異なるため）。
またRRT*はコストの最適性を漸近的に保証するアルゴリズムであり、
厳密な最適性（コストが本当に改善されること）の検証はここでは行わない。
...
def test_no_obstacles_finds_path(seed):
    ...
    obstacles = []
    path = rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                     MAX_ITERATIONS, seed)
...
def test_tiny_max_iterations_returns_empty_or_valid():
    ...
    obstacles = []
    path = rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE, 1, 42)
    if path:
        _assert_valid_path(path, START, GOAL, GOAL_RADIUS, STEP_SIZE, obstacles)
    else:
        assert path == []
...
def test_seed_reproducibility():
    ...
    obstacles = []
    path1 = rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                      MAX_ITERATIONS, 7)
    path2 = rrt_star(obstacles, BOUNDS, START, GOAL, GOAL_RADIUS, STEP_SIZE,
                      MAX_ITERATIONS, 7)
    assert path1 == path2
